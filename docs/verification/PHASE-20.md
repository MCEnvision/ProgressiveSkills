...
4. Open Skills. Confirm the upper dashboard has a total level panel, a live local player preview, and a selected skill panel. Confirm the lower selector grid shows each configured skill icon and its current synchronized level badge rather than generic text cards.
5. Hover every skill icon and confirm the preview panel changes without selecting it. Use keyboard focus on the same icons and confirm the preview follows focus. Click an icon and confirm its selected border, name, level, active XP, and banked XP remain stable after the pointer moves away.
6. Give a skill XP and levels with the existing `/pskills` test commands. Reopen Skills and confirm total level, highest track, skill count, per skill level badges, active XP, and banked XP match the synchronized command output. Page through a pack with more skills than one screen can display and confirm no icon leaves the lower inset.
